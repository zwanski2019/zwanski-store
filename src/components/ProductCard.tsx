...
export default function ProductCard({product}:{product:any}){
  ...
  const img = product.image ? (base + product.image.replace(/^\//, '')) : (base + 'placeholder.png')

  return (
    <div className='border rounded p-4 flex flex-col'>
      <a href={`/product/${product.id}`} className='block mb-4'>
        <img src={img} alt={product.title} className='h-40 object-contain w-full' />
      </a>
      ...
      <div className='mt-4 flex items-center justify-between'>
        ...
        <button onClick={handleAdd} className='bg-zwanski-blue text-white px-3 py-1 rounded'>Add</button>
      </div>
    </div>
  )
}